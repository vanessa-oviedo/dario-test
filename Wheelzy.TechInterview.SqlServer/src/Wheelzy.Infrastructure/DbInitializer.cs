...
public static class DbInitializer
{
    public static async Task EnsureCreatedAndSeedAsync(AppDbContext db, CancellationToken ct = default)
    {
        ...
        if (!await db.Statuses.AnyAsync(ct)) db.Statuses.AddRange(
        ...
        if (!await db.Makes.AnyAsync(ct))
        {
            ...
            db.Models.AddRange(
                new Model { Name = "Fiesta", Make = ford },
                new Model { Name = "Focus", Make = ford },
                new Model { Name = "Corolla", Make = toyota }
            );

            db.Submodels.AddRange(
                new Submodel { Name = "ST", Model = db.Models.Local.First(m => m.Name == "Focus") },
                new Submodel { Name = "SE", Model = db.Models.Local.First(m => m.Name == "Corolla") }
            );
        }

        if (!await db.ZipCodes.AnyAsync(ct)) db.ZipCodes.AddRange(
            new ZipCode { ZipCodeId = "33101" },
            new ZipCode { ZipCodeId = "90001" }
        );

        if (!await db.Customers.AnyAsync(ct)) db.Customers.AddRange(
            new Customer { FullName = "Alice Johnson", Email = "alice@example.com" },
            new Customer { FullName = "Bob Smith", Email = "bob@example.com" }
        );

        if (!await db.Buyers.AnyAsync(ct)) db.Buyers.AddRange(
            new Buyer { Name = "Buyer A" },
            new Buyer { Name = "Buyer B" }
        );
        ...
        if (!await db.BuyerZipQuotes.AnyAsync(ct))
        {
            ...
            foreach (var b in buyers)
                foreach (var z in zips)
                    db.BuyerZipQuotes.Add(new BuyerZipQuote { BuyerId = b.BuyerId, ZipCodeId = z.ZipCodeId, Amount = rnd.Next(500, 1500) });
        }
    ...
    public static async Task GenerateQuotesForCaseAsync(AppDbContext db, int caseId, CancellationToken ct)
    {
        ...
        var buyersForZip = await db.BuyerZipQuotes.AsNoTracking().Where(x => x.ZipCodeId == cc.ZipCodeId).ToListAsync(ct);

        foreach (var b in buyersForZip)
            db.CaseQuotes.Add(new CaseQuote { CaseId = cc.CaseId, BuyerId = b.BuyerId, Amount = b.Amount, IsCurrent = false });
    ...
    public static Task SetInitialStatusAsync(AppDbContext db, int caseId, string code, DateTime? statusDate, string changedBy, CancellationToken ct)
        => ChangeStatusAsync(db, caseId, code, statusDate, changedBy, ct);
    ...
    public static async Task ChangeStatusAsync(AppDbContext db, int caseId, string code, DateTime? statusDate, string changedBy, CancellationToken ct)
    {
        ...
        db.CaseStatuses.Add(new CaseStatus { CaseId = caseId, StatusId = status.StatusId, StatusDate = statusDate, ChangedBy = changedBy, IsCurrent = true, CreatedAtUtc = DateTime.UtcNow });
        await db.SaveChangesAsync(ct);
    }
}
